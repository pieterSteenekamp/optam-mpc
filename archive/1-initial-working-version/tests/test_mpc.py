...
class TestMPCControllerControl:
    ...
    def test_bias_correction(self):
        ...
        # Model has gain 1.0, but we'll simulate measurement with gain 1.5
        model = MIMOFOPDT(
            K=np.array([[1.0]]),
            tau=np.array([[2.0]]),
            theta=np.array([[0.0]]),
        ...
        # Simulate a few control cycles with model mismatch
        y_plant = np.array([0.0])
        u = np.array([0.0])
        
        for step in range(5):
            # Apply current input to "real plant" with gain 1.5
            y_plant = y_plant + 0.5 * 1.5 * u  # Simple integration
            y_plant = 1.5 * u  # Actually, let's use steady-state
            
            # Model thinks it should be at 1.0 * u
            y_model = 1.0 * u
            
            # Measurement from "real plant"
            result = controller.control(
                y_measured=y_plant,
            ...
            # Bias should be moving toward 0.5 (the actual mismatch)
            if step > 0:
                assert result.bias_estimate[0] > 0.0
        
        # After several steps, bias should be close to the actual mismatch
        assert abs(result.bias_estimate[0] - 0.5) < 0.2
